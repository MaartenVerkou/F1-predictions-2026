  ...
  if (existing) {
    db.prepare(
      `UPDATE race_data_snapshots
       SET round_name = ?, fetched_at = ?, source_type = ?, source_note = ?,
  ...
  const result = db
    .prepare(
      `INSERT INTO race_data_snapshots (
        season, round_number, round_name, sync_id, fetched_at, source_type,
        source_note, coverage_status, payload_json, created_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    )
    .run(
      safeSeason,
...
function linkEvidenceToActualSnapshot(db, snapshotId, evidenceId) {
  ...
  return Number(
    db
      .prepare(
        "UPDATE actual_snapshots SET source_data_snapshot_id = ? WHERE id = ?"
      )
      .run(safeEvidenceId, safeSnapshotId).changes || 0
  );
}
...
function listRaceDataSnapshots(db, season) {
  const rows = db
    .prepare(
      `SELECT id, season, round_number, round_name, sync_id, fetched_at,
              source_type, source_note, coverage_status, payload_json, created_at
...
function findEvidenceForActualSnapshot(db, snapshotId) {
  const row = db
    .prepare(
      `SELECT rds.id, rds.season, rds.round_number, rds.round_name, rds.sync_id,
              rds.fetched_at, rds.source_type, rds.source_note,
