...
- [x] 2.1 Extend question option resolution to return canonical entity options while preserving static scalar options and the existing JSON catalog contract.
- [x] 2.2 Add dual-read/canonical-write serialization for responses, guest responses, actuals, and composite answers; preserve legacy label compatibility.
- [x] 2.3 Update normalized race evidence and import derivation to persist driver/team/race IDs plus source labels and assignment metadata.
- [x] 2.4 Add a migration/report command for resolvable and unresolved legacy answer/actual/evidence values; never silently rewrite ambiguous values.

## 3. Admin Inputs workspace

- [x] 3.1 Add admin-only `/admin/inputs` route with season selector and Drivers, Teams, Assignments, Races, and Mappings tabs.
- [x] 3.2 Add protected mutations for labels, aliases, provider references, calendar state, and round-bounded assignments with validation and audit logging.
- [x] 3.3 Place Inputs before Questions in navigation and add localized labels/responsive styles.
- [x] 3.4 Show affected source/snapshot counts and unresolved mappings before potentially impactful assignment or identity changes.
